(()=>{
  ...
  let messages=[],activeMessage=null,loading=false,approved=false;
  ...
  const prettyDate=v=>new Intl.DateTimeFormat('es-ES',{weekday:'long',day:'numeric',month:'long',timeZone:'UTC'}).format(new Date(v+'T12:00:00Z'));
  ...
  async function createMessageCard(msg,forPopup=false){
    const card=document.createElement('article');card.className=forPopup?'letter letter-popup':'letter';
    const date=document.createElement('div');date.className='letter-date';date.textContent=prettyDate(msg.message_date);
    const body=document.createElement('p');body.className='letter-text';body.textContent=msg.body;card.append(date,body);
    if(msg.photo_path){const url=await signedPhoto(msg.photo_path);if(url){const img=document.createElement('img');img.className='letter-photo';img.src=url;img.alt='Fotografía que acompaña al mensaje';card.appendChild(img)}}
    if(!forPopup){
  ...
  async function renderInbox(){
    list.replaceChildren();const hidden=hiddenSet(),visible=messages.filter(m=>!hidden.has(m.id));
    if(!visible.length){list.innerHTML='<div class="inbox-empty"><span>♡</span>Todavía no hay mensajes en tu bandejita.</div>';return}
    for(const msg of visible)list.appendChild(await createMessageCard(msg));
  }
  ...
  async function loadInbox(){
    ...
    try{
      ...
      if(!approved){messages=[];updateBadge();if(!popup.hidden)closeLayer(popup);activeMessage=null;list.innerHTML='<div class="inbox-empty"><span>♡</span>Tu bandejita estará lista muy pronto. Puedes seguir disfrutando de toda la app.</div>';return}
    ...
    }catch(err){list.innerHTML='<div class="inbox-empty">No se ha podido conectar. Puedes seguir usando la app y volver a tocar el sobre para intentarlo de nuevo.</div>'}
    finally{loading=false}
  }

  $('#inboxButton').onclick=async()=>{openLayer(panel);await loadInbox();if(approved){const read=readSet();messages.filter(m=>!hiddenSet().has(m.id)).forEach(m=>read.add(m.id));saveSet('luz-inbox-read',read);updateBadge()}};
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(e.key==='Tab'){const items=[...layer.querySelectorAll('button:not(.layer-backdrop),a[href]')].filter(x=>!x.hidden);const first=items[0],last=items.at(-1);if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus()}}
  });
